...
export function AccessibilityChecker() {
  ...
  const [result, setResult] = useState<{ score: number; flags: A11yFlag[]; url: string } | null>(null);
  ...
  async function handleScan() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/tools/accessibility-scan", {
        method: "POST",
              ...
              {result.flags.map((flag, i) => (
                <div key={i} style={{ background: "#0D1B2E", border: "1px solid rgba(255,255,255,0.06)", padding: "1.5rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem", gap: "1rem" }}>
                    <p style={{ ...syne, fontSize: "14px", fontWeight: 700, color: "white" }}>{flag.title}{flag.count > 1 ? ` (${flag.count})` : ""}</p>
