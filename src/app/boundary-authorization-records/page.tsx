  ...
  return (
    <div style={{ background: "#0A1628", minHeight: "100vh" }}>
      ...
      <Section eyebrow="The record" title="Every field, and why it exists">
        ...
        <Field name="named_by_name / named_by_role">Optional. owner_name above is who holds the seat, this is who put them in it — a distinction that matters because a seat that assigns its own accountability is just marking its own homework. For a solo founder or small business there&apos;s often no one above the owner to name, and leaving this blank, or writing &quot;self assigned,&quot; is the honest answer, not a gap.</Field>
        <Field name="owner_confirmed_name / owner_confirmed_at / owner_confirmed_role">Optional. A seat named in a record nobody circulated is not accountability, it is a document produced later at a worse moment. This is the named owner&apos;s own confirmation, via a link only they can act on, that they actually know they hold it, and in what capacity — separate from the account holder asserting owner_name on their behalf.</Field>
        <Field name="owner_reconfirmed_name / owner_reconfirmed_at / owner_reconfirmed_role">Optional. owner_confirmed_at only ever answers whether the named owner once knew they held the seat, roles change, and nothing re-checked whether they still do. This is a second, later, distinct confirmation the account holder can request at any point, recorded separately from the original so both facts stay on the record rather than one overwriting the other.</Field>
        <Field name="external_dependencies">Optional. What this decision actually depends on outside the account, a vendor, a model provider, a jurisdiction, and whether losing that dependency has a tested fallback rather than an assumed one. Ownership tells you the switch could be taken away. This asks whether anyone has rehearsed the day it is.</Field>
        <Field name="warnings_overridden">Optional, and a different fact from risks_accepted above. That is the owner&apos;s own stated risk, accepted knowingly. This is a warning raised by someone else and overridden anyway, with a required reason. A clean risks_accepted list and a buried dissent look identical from outside unless the dissent has its own field.</Field>
        ...
        <Field name="reasoning_recorded">Computed once, at signing, from whether any of the three fields above held anything at that moment. Never recomputed later, so it can&apos;t be made true by editing the record afterwards. A name and a date attached to a decision is not the same fact as reasoning behind it, and an empty field used to render as nothing at all, no different on screen from a record that simply chose not to show this section. Now it&apos;s a stated fact either way: recorded, or not recorded, never silent.</Field>
        <Field name="affected_parties">Optional, and a different fact from risks_accepted and warnings_overridden above. Those describe risk to the business making the decision. This describes who is downstream of it and what it costs them, the person whose timeline doesn&apos;t match the one the business reports on. Self reported, not independently confirmed, because the point is giving it somewhere to exist at all.</Field>
        <Field name="impact_disclosed">Computed once, at signing, from whether affected_parties held anything at that moment. Same treatment as reasoning_recorded. Most records will read as not disclosed, honestly, because most decisions never had anyone downstream named. That is the finding this field exists to make visible, not a defect in the field.</Field>
      </Section>
