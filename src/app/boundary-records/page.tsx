...
import type { Plan, BoundaryAuthorizationRecord, BoundaryOption, BoundaryRisk, BoundaryEvidence } from "@/types";
...
function NewRecordForm({ onCreated }: { onCreated: (record: BoundaryAuthorizationRecord) => void }) {
  ...
  const canSubmit = decision.trim() && ownerName.trim() && ownerRole.trim() && decisionDate.trim();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[rgba(244,241,234,0.5)]">Six fields, one record: decision, named owner, options considered, risks accepted, evidence relied on, and date. Log it once and it stands as your governance trail.</p>
      </div>
