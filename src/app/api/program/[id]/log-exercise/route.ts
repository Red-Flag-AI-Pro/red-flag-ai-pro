...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { documentKey, note } = (body ?? {}) as { documentKey?: unknown; note?: unknown };
  ...
  const documentReviews = { ...(order.document_reviews as Record<string, { reviewed_at?: string; exercised_at?: string; exercise_note?: string }> ?? {}) };
  const existing = documentReviews[EXERCISABLE_DOCUMENT_KEY] ?? { reviewed_at: nowIso };
  documentReviews[EXERCISABLE_DOCUMENT_KEY] = { ...existing, exercised_at: nowIso, exercise_note: note.trim().slice(0, 2000) };
  ...
  const verifyId = await logAuditEvent(
    ...
    { program_order_id: id, document_key: EXERCISABLE_DOCUMENT_KEY, note: note.trim().slice(0, 2000) },
    { timestamp: true }
  );
