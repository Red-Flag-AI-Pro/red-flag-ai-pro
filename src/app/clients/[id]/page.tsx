  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <Link href="/clients" className="text-xs text-gray-400 hover:text-gray-600 mb-1 block">
            ← Clients
          </Link>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          ...
          <p className="mt-1 text-3xl font-bold text-gray-900">{scans?.length ?? 0}</p>
        </Card>
        <Card>
      ...
      <Card padding="none">
        ...
        {!scans || scans.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-gray-400 text-sm">No scans for this client yet.</p>
            <Link href={`/scans/new?client=${id}`} className="mt-2 inline-block text-sm text-red-600 hover:underline">
              Run the first scan →
            </Link>
          </div>
        ) : (
